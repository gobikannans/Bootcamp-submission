...
export default function SubmissionForm(){
  ...
  const [otherLinksError,setOtherLinksError]=useState(false)
  ...
  const sd=new Date(startDate)
    ...
    if(sd>ed || ed<sd){
      alert("Please give proper start date and end date") 
    }
  ...
  const onSubmitForm=(event)=>{
    ...
    if ("https://github.com/"!==githubUrl.substring(0,19)){
      setGithubError(true)
      setTimeout(()=>setGithubError(false),5000)
    }else if(validator.isURL(otherLinks)!==true){
      setOtherLinksError(true)
      setTimeout(()=>setOtherLinksError(false),5000)
    }else if(title==="" || summary==="" || description==="" || image==="" || name==="" || startDate==="" || endDate==="" || githubUrl==="" || otherLinks===""){
        setFormError(true)
                ...
                <div className="form-container">
                    ...
                    <form className="main-container"   onSubmit={onSubmitForm}>
                      ...
                      <div className="form-item-container">  
                        ...
                        <p className="error-msg">{githubError? "*Please provide valid Github Url":null} </p> 
                      </div>
                      <div className="form-item-container">  
                        ...
                        <p className="error-msg">{otherLinksError? "*Please provide valid demo Url":null} </p> 
                      </div>
                      <button className="form-btn" type="submit" >Upload Submission</button>
                      <p className="error-msg-form">{formError? "*Please fill all the required Details":null} </p>
                    </form>
                </div>
            </div>
            </>
